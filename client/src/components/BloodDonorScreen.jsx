...
const emptyRequest = { bloodGroup: "O+", units: 1, hospitalId: "", hospitalName: "", notes: "" };
...
export default function BloodDonorScreen({ currentUser, onAddRequest, onBecomeDonor }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/hospitals")
      .then(async (response) => {
        ...
        if (!response.ok) throw new Error(data.detail || "Could not load hospitals.");
        if (active) setHospitals(Array.isArray(data) ? data : []);
      })
  ...
  const submitRequest = async () => {
    if (!requestForm.hospitalId || !requestForm.hospitalName.trim()) return;
